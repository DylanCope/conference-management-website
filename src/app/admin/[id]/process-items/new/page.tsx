...
export default async function NewProcessItemPage({ params }: { params: { id: string } }) {
  ...
  <p style={{ color: 'var(--muted)', marginTop: 4 }}>Create a process item with full details.</p>
  ...
  <input name="title" placeholder="e.g., Internal review" style={{ width: '100%', padding: 8, border: '1px solid var(--border)', borderRadius: 4, background:'var(--card)', color:'var(--text)' }} />

        <label style={{ display: 'block', marginTop: 10 }}>Description</label>
  <textarea name="description" placeholder="Optional longer description or checklist" rows={4} style={{ width: '100%', padding: 8, border: '1px solid var(--border)', borderRadius: 4, background:'var(--card)', color:'var(--text)' }} />

  <label style={{ display: 'block', marginTop: 10 }}>Due date</label>
  <input type="date" name="dueDate" style={{ width: 220, padding: 8, border: '1px solid var(--border)', borderRadius: 4, background:'var(--card)', color:'var(--text)' }} />
